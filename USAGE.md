...
6. [Real-time Bridge (Redis)](#6-real-time-bridge-redis)
7. [Feature Engineering](#7-feature-engineering)
8. [Cấu trúc lưu trữ Parquet](#8-cấu-trúc-lưu-trữ-parquet)
9. [Sử dụng trực tiếp trong Python](#9-sử-dụng-trực-tiếp-trong-python)
10. [Chạy tests](#10-chạy-tests)
11. [Offline mode (không cần MT5)](#11-offline-mode-không-cần-mt5)
...
Hệ thống gồm **2 process riêng biệt**, có thể chạy song song:

| Process | Lệnh | Mục đích |
|---|---|---|
| REST API | `uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload` | Download history, query data |
| Bridge | `python bridge.py` | Real-time streaming + order execution |
...
## 6. Real-time Bridge (Redis)

Bridge stream data real-time từ MT5 qua Redis, hoàn toàn tách biệt với REST API.

### Yêu cầu thêm

```bash
# Cài Redis dependency
...
REDIS_TICK_MAXLEN=50000   # số ticks tối đa giữ trong stream
REDIS_BAR_MAXLEN=10000    # số bars tối đa giữ trong stream
```
...
Bridge tự động:
1. Connect MT5 terminal
2. Connect Redis
3. Spawn các tasks song song cho mỗi symbol trong `DEFAULT_SYMBOLS`

### Redis streams được publish

| Stream | Nội dung | Tần suất |
|---|---|---|
| `stream:ticks:{symbol}` | bid, ask, spread | ~100ms |
| `pubsub:ticks:{symbol}` | giống trên, fire-and-forget | ~100ms |
| `stream:bars:{symbol}:M5` | OHLCV closed bar | mỗi khi bar đóng |
| `stream:bars:{symbol}:M15` | OHLCV closed bar | mỗi khi bar đóng |
| `stream:bars:{symbol}:H1` | OHLCV closed bar | mỗi khi bar đóng |
| `stream:fills` | kết quả thực thi lệnh | sau mỗi order |

### Gửi order từ trading bot

```bash
redis-cli XADD stream:orders '*' \
  id "order-001" symbol "XAUUSDm" side "BUY" \
  volume "0.1" sl "2990.0" tp "3040.0"
```
...
```bash
redis-cli XREAD COUNT 10 STREAMS stream:fills 0
```
...
### Theo dõi real-time (subscribe pub/sub)

```bash
# Ticks real-time (low latency)
redis-cli SUBSCRIBE pubsub:ticks:XAUUSDm

# Bar closes
redis-cli XREAD COUNT 5 STREAMS stream:bars:XAUUSDm:M5 0
```

### Metrics spread/slippage

Bridge tự flush metrics mỗi 10 giây vào Redis hash:

```bash
redis-cli HGETALL metrics:spread:XAUUSDm
# avg 0.12  max 0.45  samples 1000

redis-cli HGETALL metrics:slippage:XAUUSDm
# avg 0.02  max 0.18  samples 340
```
